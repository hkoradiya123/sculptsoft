...
def list_inventories(session: Session = Depends(get_db)):
...
def create_inventory(payload: InventoryCreate, session: Session = Depends(get_db)):
...
def get_inventory(inventory_id: int, session: Session = Depends(get_db)):
...
def delete_inventory(inventory_id: int, session: Session = Depends(get_db)):
    inventory = get_inventory_or_404(session, inventory_id)
...
def list_locations(inventory_id: int, session: Session = Depends(get_db)):
...
def create_product(payload: ProductCreate, session: Session = Depends(get_db)):
...
def list_stock(inventory_id: int, session: Session = Depends(get_db)):
    get_inventory_or_404(session, inventory_id)
    rows = (
